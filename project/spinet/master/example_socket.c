...
static void mqtt_task(void *param)
{
	...
	char *big_data = luat_heap_opt_malloc(LUAT_HEAP_AUTO, 64 * 1024);
	...
	while(1){
		if (luat_mqtt_state_get(mymqtt) == MQTT_STATE_READY){
			...
			ret = mqtt_publish_with_qos(&(mymqtt->broker), mqtt_pub_topic, big_data, 16 * 1024, 1, MQTT_DEMO_PUB_QOS, &message_id);
