...
void network_uplink(const uint8_t* data, size_t len) {
	size_t tx_len;
	if (g_s_network_ctrl) {
		network_tx(g_s_network_ctrl, data, len, 0, NULL, 0, &tx_len, 0);
	}
}
...
static void luat_test_task(void *param)
{
	...
	*/
	...
	const char remote_ip[] = "netlab.luatos.com";
	int port = 45877;
						...
						if (!is_timeout && !is_break)
						{
							do
							{
								...
								if (rx_len > 0)
								{
									...
									LUAT_DEBUG_PRINT("rx data %.*s", rx_len, rx_data);
									luat_uart_write(1, rx_data, rx_len);
								}
