...
#define LFS_BLOCK_DEVICE_TOTOAL_SIZE    (FLASHEX_FS_REGION_SIZE)
        ...
        lfs_off_t off, void *buffer, lfs_size_t size)
{
    ...
    spiFlashRead((FLASHEX_FS_REGION_START + block * cfg->block_size + off), (uint8_t *)buffer, size);
        ...
        lfs_off_t off, const void *buffer, lfs_size_t size)
{
    ...
    spiFlashWritePage((FLASHEX_FS_REGION_START + block * cfg->block_size + off), (uint8_t *)buffer, size);
...
static int block_device_erase(const struct lfs_config *cfg, lfs_block_t block)
{
    ...
    spiFlashEraseSector(FLASHEX_FS_REGION_START + block * cfg->block_size);
