 ...
 * File name:    timer.c
 * Description:  EC718 timer driver source file
...
#define TIMER_TEXT_SECTION     SECTION_DEF_IMPL(.sect_timer_text)
#define TIMER_RODATA_SECTION   SECTION_DEF_IMPL(.sect_timer_rodata)
#define TIMER_DATA_SECTION     SECTION_DEF_IMPL(.sect_timer_data)
#define TIMER_BSS_SECTION      SECTION_DEF_IMPL(.sect_timer_bss)
...
TIMER_BSS_SECTION static bool   gIsTimerDriverInited = false;

TIMER_BSS_SECTION static TimerDatabase_t gTimerDataBase[TIMER_INSTANCE_NUM];

#endif

TIMER_DATA_SECTION static ClockId_e gTimerClocks[TIMER_INSTANCE_NUM*2] =   {PCLK_TIMER0, FCLK_TIMER0,
 ...
 */
TIMER_BSS_SECTION static uint32_t gTimerWorkingStatus;
...
void TIMER_init(uint32_t instance, const TimerConfig_t *config)
{
    ...
    TIMER_driverInit();
     ...
     if(gTimerDataBase[instance].isInited == false)
     {
	    ...
	    if(config->captureMode == TIMER_CAPTURE_MODE_ENABLE)
	    {
	        EIGEN_TIMER(instance)->TCTLR = EIGEN_VAL2FLD(TIMER_TCTLR_MODE, TIMER_INTERNAL_CLOCK) | \
	                                       EIGEN_VAL2FLD(TIMER_TCTLR_CAPTURE_EN, TIMER_CAPTURE_MODE_ENABLE) | \
	                                       EIGEN_VAL2FLD(TIMER_TCTLR_EDGE_TYPE, config->captureEdge);
	    }
	    else
	    {
...
void TIMER_setExternalEdgeCountType(uint32_t instance, TimerCaptureEdge_e type)
{
    ...
    EIGEN_TIMER(instance)->TCTLR = (EIGEN_TIMER(instance)->TCTLR &~ TIMER_TCTLR_EDGE_TYPE_Msk) | EIGEN_VAL2FLD(TIMER_TCTLR_EDGE_TYPE, type);
}

void TIMER_setExternalClockInput(uint32_t instance, uint32_t gpioPort, uint32_t gpioPin, bool onOff)
{
    extern void GPR_setTimerInputPin(uint32_t timerInstance, uint32_t gpioIdx, bool onOff);
...
void TIMER_getCaptureResult(uint32_t instance, TimerCaptureResult_t* result)
{
    ...
    if(result)
    {
        result->counterValue = EIGEN_TIMER(instance)->TCVR;
        result->capturedEdge = EIGEN_TIMER(instance)->TCER;
    }
}
...
void TIMER_interruptConfig(uint32_t instance, TimerInterruptSource_e source, TimerInterruptConfig_e config)
{
    ...
    uint32_t enableMask = 0, typeMask = 0, registerValue = EIGEN_TIMER(instance)->TCTLR;

    switch(source)
    {
        case TIMER_MATCH0_INTERRUPT:

            enableMask = TIMER_TCTLR_IE_0_Msk;
            typeMask = TIMER_TCTLR_IT_0_Msk;

            break;
        case TIMER_MATCH1_INTERRUPT:

            enableMask = TIMER_TCTLR_IE_1_Msk;
            typeMask = TIMER_TCTLR_IT_1_Msk;

            break;
        case TIMER_MATCH2_INTERRUPT:

            enableMask = TIMER_TCTLR_IE_2_Msk;
            typeMask = TIMER_TCTLR_IT_2_Msk;

            break;
        case TIMER_CAPTURE_INTERRUPT:

            enableMask = TIMER_TCTLR_CAPTURE_EN_Msk;
            typeMask = TIMER_TCTLR_CAPTURE_IT_Msk;
...
TimerInterruptConfig_e TIMER_getInterruptConfig(uint32_t instance, TimerInterruptSource_e source)
{
    ...
    if ((enableMask & registerValue) == 0)
        return TIMER_INTERRUPT_DISABLE;
