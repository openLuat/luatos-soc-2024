 ...
 * File name:    adc.c
 * Description:  EC718 adc driver source file
 * History:
 *
 ****************************************************************************/
...
#define ADC_TEXT_SECTION     SECTION_DEF_IMPL(.sect_adc_text)
#define ADC_RODATA_SECTION   SECTION_DEF_IMPL(.sect_adc_rodata)
#define ADC_DATA_SECTION     SECTION_DEF_IMPL(.sect_adc_data)
#define ADC_BSS_SECTION      SECTION_DEF_IMPL(.sect_adc_bss)
...
ADC_BSS_SECTION static AdcReqeustQueue_t gAdcRequestqueue;

/**
   \brief variable for keeping track the index of current request bitmap, see below bitmap layout
 */
ADC_BSS_SECTION static volatile uint32_t gCurrentRequestIndex;
...
ADC_BSS_SECTION static AdcDatabase_t g_adcDataBase = {0};
...
static void ADC_loadConfigAndStartConversion(uint32_t requestIndex)
{
    ...
    ADC->AIOCFG = (ADC->AIOCFG & ADC_AIOCFG_VREF2AIO1_EN_Msk) | g_adcDataBase.channelDataBase[requestIndex].configRegisters.AIOCFG;
    ADC->CFG = g_adcDataBase.channelDataBase[requestIndex].configRegisters.CFG;

    // start conversion
    ADC->CTRL = ADC_CTRL_LDO_EN_Msk | ADC_CTRL_EN_Msk | ADC_CTRL_FC_Msk;
    delay_us(5);
...
int32_t ADC_channelInit(AdcChannel_e channel, AdcUser_t userID, const AdcConfig_t *config, adcCallback_t callback)
{
    ...
    configValue |= ( EIGEN_VAL2FLD(ADC_CFG_WAIT_CTRL, 1) | \
                     ...
                     EIGEN_VAL2FLD(ADC_CFG_VCM_SEL, 4)
                );
